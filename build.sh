...
#   dev    — compile + unit tests only (integration tests skipped, no Docker required)
#   stage  — compile + unit tests + integration tests (*IT.java via Failsafe, Docker required)
...
#   *IT.java    →  Failsafe  (integration tests, run only in stage)
#
# To run all tests locally:
#   ./build.sh stage
# To run a specific test class manually:
#   mvn test -Dtest=ProductServiceTest
#   mvn verify -Pstage -Dit.test=ProductControllerIT
...
case "$PROFILE" in
    ...
    local)
        echo "==> [local] Running unit tests only"
        ./mvnw test -Plocal
        ;;

    dev)
        echo "==> [dev] Running unit tests only (integration tests skipped)"
        ./mvnw test -Pdev
        ;;
